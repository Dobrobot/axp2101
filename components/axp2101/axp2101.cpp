...
static esphome::axp2101::AXP2101Component *pmu_component = nullptr;
...
static int pmu_read(uint8_t devAddr, uint8_t regAddr, uint8_t *data, uint8_t len)
{
    if (pmu_component == nullptr) {
        return -1;
    }
    return pmu_component->read_bytes(regAddr, data, len) ? 0 : -1;
}

static int pmu_write(uint8_t devAddr, uint8_t regAddr, uint8_t *data, uint8_t len)
{
    if (pmu_component == nullptr) {
        return -1;
    }
    return pmu_component->write_bytes(regAddr, data, len) ? 0 : -1;
}
...
{
    pmu_component = this;
    if (!PMU.begin(this->address_, pmu_read, pmu_write)) {
    ...
    switch (this->model_) {
      case AXP2101_M5CORE2:
      {
        if (brightness_ <= 0.01f) {
          ...
          uint16_t mv = brightness_to_bldo1_mv_(brightness_);
          PMU.enableBLDO1();
          PMU.setBLDO1Voltage(mv);
          ESP_LOGD(TAG, "BLDO1 voltage=%u mV", mv);
        }
        break;
      }
    }
...
void AXP2101Component::set_backlight_enabled(bool enabled)
{
    if (!initialized_) {
        return;
    }
...
uint16_t AXP2101Component::brightness_to_bldo1_mv_(float brightness)
{
    const float min_fraction = 0.65f;
    ...
    brightness = min_fraction + (brightness * (1.0f - min_fraction));
    int step = static_cast<int>(brightness * 30.0f + 0.5f);
    if (step < 0) {
        step = 0;
    }
    if (step > 30) {
    ...
    return static_cast<uint16_t>(500 + step * 100);
}
